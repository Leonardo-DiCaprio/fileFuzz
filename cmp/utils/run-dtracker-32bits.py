...
def do_dtracker(command_line, cmd_args, taint_log_dir):
    ...
    dtracker_output = os.path.join(os.getcwd(), 'pintool.log')
...
def main():
    ...
    assert os_bits == '32bit', 'DTracker is a 32-bit only program, and cannot run on %s systems.' % os_bits 
    
    pin_exec = os.path.join(args.dtracker_root, 'pin' + os.sep + 'pin.sh')
    assert os.path.exists(pin_exec), 'Cannot find the Pintool script %s.' % pin_exec

    dtracker_exec = os.path.join(args.dtracker_root, 'obj-ia32' + os.sep + 'dtracker.so')
    ...
    cmds = pin_exec + ' -injection child -follow_execv -t ' + dtracker_exec + ' -- ' + target_prog + ' ' 
