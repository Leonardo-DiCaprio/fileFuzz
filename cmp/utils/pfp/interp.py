...
class PfpInterp(object):
    ...
    def _handle_node(self, node, scope=None, ctxt=None, stream=None):
        ...
        #print '_node_switch:', node.__class__
        try:
          ...
          return res
        #except Exception,e:
        except  errors.InvalidArguments,e:
          #print e
          raise errors.InterpReturn(0)
    ...
    def _handle_decl(self, node, scope, ctxt, stream):
        ...
        self._check_mem_counter +=1
        if self._check_mem_counter % 1000 == 0 :
#           print '_handle_decl: ',self._check_mem_counter
            p = psutil.Process()
            pmm=p.memory_full_info()
            vmm= pmm.uss
#           print '_handle_node', vmm
            if vmm > (1024*1024*1024*0.25) :
            # if vmm > (1024*1024*100) :
                print "Give up: consume too much memory:", vmm
#               raise Exception("Memory exceed limits")
                raise errors.InterpReturn(-1)
        ...
        # stream!
            ...
            #print 'local var:',field_name,field
            if not isinstance(field, fields.Field) and not is_struct:
                field = field()
            else:
                field = field()
                #print 'child map:', field._pfp__children_map
            scope.add_local(field_name, field)
            #print 'local var2:', field_name, field
        ...
        #     };
            ...
            pass
        #print 'before return:',type(field),field
        return field
    ...
    def _handle_binary_op(self, node, scope, ctxt, stream):
        ...
        def eval(nd):
            return self._handle_node(nd, scope, ctxt, stream)

        def and_op(x,y):
            if not eval(x): return False
            return eval(y)
        def or_op(x,y):
            if eval(x) :
                return True
        ...
        res = switch[node.op](node.left, node.right)
    ...
    def _handle_func_call(self, node, scope, ctxt, stream):
       ...
       #print 'call:',node.name.name, node.args
       try:
           v=func.call(func_args, ctxt, scope, stream, self, node.coord)
           return v
       except Exception,e:
           print e
           raise errors.InterpReturn(-1)
