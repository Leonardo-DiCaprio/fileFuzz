...
class Struct(Field):
    ...
    def _pfp__set_value(self, value):
        ...
        if isinstance(value,Struct):
            #print 'value=',value._pfp__show()
            self._pfp__children=[]
            for x in six.moves.range(len(value._pfp__children)):
                #print 'copy child:',x,value._pfp__children[x]
                self._pfp__children.append (value._pfp__children[x])
            self._pfp__children_map = value._pfp__children_map
            self._pfp__offset = value._pfp__offset
            #print 'value=', self._pfp__show()
            return
    ...
    #def __len__(self):
    #    return len(self._pfp__children)
            ...
            for other_child in self._pfp__children:
                ...
                if isinstance(other_child, Array) and other_child.is_stringable():
                    ...
                    try:
                        ...
                        print 'I cannot update the field. may run to the end of stream(Mutation hack-me?):',str(e)
                        pass
                new_stream.seek(0)
...
class NumberBase(Field):
    ...
    intsize={'b':(-128,127),'c':(-128,127),'B':(0,255),'h':(-32768,32767),'H':(0,65535),
                  'i':(-2147483648L,2147483647L),'I':(0,4294967295),
                  'l':(-2147483648L,2147483647L),'L':(0,4294967295),
                  'q':(-9223372036854775808L,9223372036854775807L ),'Q':(0,18446744073709551615L)}
    ...
    def _pfp__build(self, stream=None, save_offset=False):
        ...
        if stream is not None and save_offset:
            self._pfp__offset = stream.tell()
#       print '_pfp__build',self._pfp__value,self._pfp__width(),self.format,self.bitsize
        tmpformat=self.format
        if tmpformat.lower() != tmpformat:
             if self._pfp__value <0 :
                 v2= struct.pack(tmpformat.lower(), self._pfp__value)
                 v3= struct.unpack(tmpformat,v2)
                 self._pfp__value = v3[0]
             #    print '_pfp__build:rebuild',v3
        if self.format in ['c','b','B','h','H','i','I','l','L','q','Q']:
             
                maxv=self.intsize[self.format][1]
                minv=self.intsize[self.format][0]
                if maxv < self._pfp__value :
                      self._pfp__value = maxv
                if minv > self._pfp__value:
                      self._pfp__value = minv
#        print '_pfp__build:final:',self._pfp__value 
...
class IntBase(NumberBase):
    ...
    def _pfp__set_value(self, new_val):
        ...
        if isinstance(new_val, IntBase):
            ...
            if new_val.signed and new_val._pfp__value < 0:
                pchar=b"\xff"
            else:
                pchar=b"\x00"
            ...
            if isinstance(new_val, NumberBase):
                new_val = int(new_val._pfp__value)
            mask = 1 << (8*self.width)
...
class Array(Field):
    ...
    def _array_to_str(self, max_len=-1):
        ...
        for item in self.items:
            ...
            # TODO WChar
            res += chr(PYVAL(item)&0xff) #just a trade-off. TODO:convert it to real wide char/str
        return res
