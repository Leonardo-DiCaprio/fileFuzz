...
def hex_string_bit_add(x,bit): #hex string:0xdhjakhdL
    s = x[2:-1]
    add_length = bit - len(s)

    num = add_length*'0'+s

    final = "0x"+num
    return final
def hex_string_bit_add_with_L(x,bit): #hex string:0xdhjakhdL
    s = x[2:-1]
    add_length = bit - len(s)
...
def get_addr_range(prog, binary):
    ...
    for k in prog.loader.shared_objects.keys():
        print(prog.loader.shared_objects[k])
...
def cfg_navigation(binary):
    ...
    new_loc2addr = defaultdict(list)  # loc -> [(prev_addr, cur_addr), ...]
    for loc in loc2addr:
        for cmpad in loc2addr[loc]:
            cmpad = list(cmpad)
            cmpad[0] = hex_string_bit_add(cmpad[0],16)
            if len(cmpad)>1:
                cmpad[1] = hex_string_bit_add(cmpad[1],16)
            cmpad = tuple(cmpad)
            new_loc2addr[loc].append(cmpad)
    with open(l2a_outf, "w") as ouf:
        json.dump(new_loc2addr, ouf)

    for loc in loc2cmpaddr:
        y = loc2cmpaddr[loc]
        for i in range(len(y)):
            y[i] = hex_string_bit_add_with_L(y[i],16)
