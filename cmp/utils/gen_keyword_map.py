...
def read_taint_file_for_64bit_taint(taintfile):
    global ins2offsets
    ...
    with open(taintfile) as tf:
        for line in tf.readlines():
            ...
            ins = line[:pos].strip()
            value = line[rpos+1:].strip()
            middle = line[pos:rpos+1].strip()
            if len(ins.split(" "))!=4:
                continue
            bit_width,op1,op2,addr = ins.split(" ")
            value1,value2 = value.split(" ")

            ins_key = addr+":"+bit_width+","+op1+","+op2+","+value1+","+value2
            import re
            # print middle
            off_set = set()
            for offset in middle.split("}"):
                # print offset
                off = offset.strip()
                pattern = re.compile(r"\{(.*)")
                groups = re.findall(pattern,off)
                if len(groups)>0:
                    group = groups[0]
                    if group!="":
                        # print group
...
def map_variable_len(struct_buf, t, last_kw, cur_kw):
    ...
    if t==len(struct_buf)-1:
        last = kw_pos 
    else:
        last = struct_buf[t+1]
    end_pos = last - 1 - kw_pos
...
def gen_keyword_map(cur_keywords):
    ...
    for p in sorted(cur_keywords.keys()):
        if p not in offset2struct.keys():
            print("execption exit...")
            exit(1)
    ...
    for p in sorted(offset2struct.keys()):
        ...
        if p in cur_keywords:
            length_from_cur_keywords = int(len(cur_keywords[p][1])/2)
            if length!=length_from_cur_keywords:
                print("length different from cur_kerwords and struct2offset,set it to cur kerwors length")
                length = length_from_cur_keywords
        if head in kw_heads:
...
def do_mapping(corpus_dir, filename=""):
    ...
    if not filename:
        for cf in os.listdir(corpus_dir):
            ...
            read_taint_file_for_64bit_taint(t_fpath)  # generate ins2offsets
        ...
        if len(cur_keywords)<1:
            print("cur_keywords is null and exit...")
            exit(0)
        gen_keyword_map(cur_keywords)
        passed += 1
        return passed
...
def main():
    ...
    parser.add_argument('--keyword_file', default='./kewords_png.txt', action='store', type=str, help='specify the keyword file.')
    ...
    global houzhui
    kd_f = args.keyword_file
    houzhui = kd_f.split(".")[0].split("_")[-1]
